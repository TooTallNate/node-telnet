...
function TelnetSocket (input, output) {
  Stream.call(this)
...
require('util').inherits(TelnetSocket, Stream)
module.exports = exports = TelnetSocket

;['do','dont','will','wont'].forEach(function (command) {
  function get () {
    return new Command(command, this)
  }
  Object.defineProperty(TelnetSocket.prototype, command, {
      get: get
    , enumerable: true
    , configurable: true
  })
...
function Command (command, client) {
  ...
  this.client = client
  debug('creating Command instance', command, this.command)
}
...
Object.keys(OPTIONS).forEach(function (name) {
  ...
  Command.prototype[name.toLowerCase()] = function () {
    ...
    buf[1] = this.command
    buf[2] = code
    debug('writing Command', name, buf)
...
exports.createServer = function (cb) {
  ...
  function conn (socket) {
    var client = new TelnetSocket(socket, socket)
