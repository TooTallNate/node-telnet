 ...
 *
 *	Revisions:
 *		2015-02-25	Darron Smith (darron@xara.com) - Added guard code to fix the problem with crashing on invalid commands.
 */
...
function parseCommand (bufs, i, event) {
  ...
  if (typeof COMMAND_IMPLS[command] !== 'function') {
		event.buf = bufs.splice(0, i + 1).toBuffer();
		event.commandCode = null;
		event.command = null;
		return event;
  }
...
function parseOption (bufs, i, event) {
  ...
  if (typeof OPTION_IMPLS[option] !== 'function') {
		event.buf = bufs.splice(0, i + 1).toBuffer();
		event.optionCode = null;
		event.option = null;
		return event;
  }
