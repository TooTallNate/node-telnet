...
Client.prototype.open = function() {
  ...
  this.input.on('data', function (data) {
    var bufs = [];
    for (var i = 0, l = 0; i < data.length; i++) {
      if (data.readUInt8(i) === COMMANDS.IAC
          && data.length - 1 - i >= 2
          && COMMAND_NAMES[data.readUInt8(i + 1)]
          && OPTION_NAMES[data.readUInt8(i + 2)]) {
        var cdata = data.slice(i);
        var iacCode = cdata.readUInt8(0);
        var iacName = COMMAND_NAMES[iacCode];
        var commandCode = cdata.readUInt8(1);
        var commandName = COMMAND_NAMES[commandCode];
        var optionCode = cdata.readUInt8(2);
        var optionName = OPTION_NAMES[optionCode];
        var cmd = {
          iacCode: iacCode,
          iacName: iacName,
        ...
        console.error(cmd);
        var len = self[cmd.optionName](cmd);
        ...
        if (i === data.length - 1 || data[i + 1] === COMMANDS.AIC) {
...
Client.prototype.environment_variables = function(cmd) {
  ...
  //console.error(data);
  //console.error(Array.prototype.slice.call(data));
